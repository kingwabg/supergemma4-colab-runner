...
항상 JSON 객체 하나로만 도구를 호출하세요. 설명이나 코드 펜스를 붙이지 마세요.
허용 도구:
{"action":"inspect"}
{"action":"read_files","paths":["경로"]}
{"action":"write_files","files":{"경로":"전체 파일 내용"}}
{"action":"run_tests"}
{"action":"finish","summary":"변경 요약"}
작업공간 밖 접근, 셸 명령, 네트워크, 파일 삭제는 허용되지 않습니다.
먼저 파일을 확인하고, 요구사항에 필요한 최소 변경만 하며, 가능하면 공개 테스트를 실행한 뒤 완료하세요."""


def _execute_action(workspace: Path, action: dict[str, Any]) -> tuple[str, bool, bool]:
...
def run_codex_task(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix=f"supergemma-{case['id'].lower()}-") as directory:
        ...
        for step_number in range(1, max_steps + 1):
            ...
            try:
                ...
                tool_result, finished, policy_blocked = _execute_action(workspace, action)
    ...
    passed = bool(finished and hidden_result["passed"] and protocol_errors <= 1 and policy_blocks == 0)
    return {
        "id": case["id"],
